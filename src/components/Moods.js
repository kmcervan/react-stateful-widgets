...
export default function Moods() {
  ...
  return (
    <div className='widget-moods container'>
      ...
      <div>
        <button id='makeHappy' onClick={(event) => setMood(happyMood)}>Make Happy</button>
        <button id='makeSad' onClick={(event) => setMood(sadMood)}>Make Sad</button>
        <button id='resetMood' onClick={(event) => setMood(initialMood)}>Reset</button>
      </div>
    </div>
  );
}
